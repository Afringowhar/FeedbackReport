...
#             # Then create PDF task that depends on HTML
#             pdf_task_id = str(uuid.uuid4())
#             ReportTask.objects.create(task_id=pdf_task_id)
...
#             return Response({"task_id": pdf_task_id}, status=status.HTTP_202_ACCEPTED)
...
#                 # Handle single student or batch processing
#                 if isinstance(data, list):
#                     # For multiple students in one file
#                     return self._handle_batch_upload(data)
#                 return data
...
#         # Handle direct JSON payload
#         data = request.data
#         if not all(key in data for key in ['student_id', 'events']):
#             return Response(
#                 {"error": "Invalid payload format. Required fields: student_id, events"},
#                 status=status.HTTP_400_BAD_REQUEST
#             )
#         return data

#     def _handle_batch_upload(self, student_data_list):
#         if len(student_data_list) > 0:
#             return student_data_list[0]
#         return Response(
#             {"error": "Empty student data list"},
#             status=status.HTTP_400_BAD_REQUEST
#         )
